...
The normal conversation surface is a provider-neutral MCP stdio server with eight bounded tools for
status, refresh, catalog search, watchlist inspection and update, inbox inspection and update, and
inbox explanations. Connection, credential storage, schedules, import, restore, backup, and data
deletion remain local CLI operations.
